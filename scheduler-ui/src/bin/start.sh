...
PID_FILE='PID'
SPRING_OPTS=" --spring.pid.file=$PID_FILE"
cd `dirname $0`
BIN_DIR=`pwd`
cd ..
DEPLOY_DIR=`pwd`
...
nohup /export/servers/jdk1.8.0_60/bin/java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -jar $DEPLOY_DIR/$JAR_NAME $SPRING_OPTS > $STDOUT_FILE 2>&1 &
PC=0
COUNT=0
while [ $COUNT -lt 1 ]; do
    echo -e ".\c"
